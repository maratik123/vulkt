...
impl HelloTriangleApplication {
    pub fn new(validate: bool) -> AppResult<HelloTriangleApplication> {
        let (event_loop, window) = HelloTriangleApplication::init_window()?;
        let instance = HelloTriangleApplication::init_vulkan(&event_loop, validate)?;
        let _debug_utils_messenger = if validate {
            Some(HelloTriangleApplication::setup_debug_messenger(
                instance.clone(),
            )?)
        } else {
            None
    ...
    fn init_vulkan(event_loop: &EventLoop<()>, validate: bool) -> AppResult<Arc<Instance>> {
        HelloTriangleApplication::create_instance(event_loop, validate)
    }
    ...
    fn create_instance(event_loop: &EventLoop<()>, validate: bool) -> AppResult<Arc<Instance>> {
        ...
        if validate {
            let available_layers: HashSet<_> = library
                .layer_properties()
                .unwrap()
                ...
                ..DebugUtilsMessengerCreateInfo::user_callback(unsafe {
                    DebugUtilsMessengerCallback::new(
                        |message_severity, message_type, callback_data| {
                            warn!(
                                "validation layer {message_severity:?}-{message_type:?}: {}",
